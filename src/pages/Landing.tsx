// src/pages/Landing.tsx - Versión completa y fixada con export default
import React from 'react';
import { Link } from 'react-router-dom'; // Para los links a login/registro

export default function Landing() {  // ← ¡ÉSTA ES LA CLAVE! Export default como 'Landing'
  const teamMembers = [
    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {projectSections.map((section, index) => (
              <div
                key={section.id}
                className="bg-[#3B82F6] shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-2xl rounded-lg overflow-hidden"
                style={{ animationDelay: `${index * 100}ms` } as React.CSSProperties}  // Fix para inline style warning
              >
                <div className="p-6">
      ...
      <section className="px-6 py-16">
        <div className="mx-auto max-w-6xl text-center">
          <Link to="/login">
            <button
              className="animate-pulse bg-[#3B82F6] px-12 py-8 text-2xl font-bold text-white shadow-2xl transition-all hover:scale-110 hover:bg-[#10B981] rounded-md mr-4"
            >
              Iniciar Sesión
            </button>
          </Link>
